  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4">
      <div className="bg-surface-1 border border-surface-3 rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4 max-h-[90vh] overflow-y-auto">
        <h2 className="text-lg font-semibold text-gray-100">Bill #{order.bill_number}</h2>
        ...
        {billCalcs.length > 0 && (
          <div className="space-y-1">
            {billCalcs.map(c => {
              const previewAmt = c.type === 'percentage' ? subtotal * (c.value / 100) : Number(c.value);
              return (
                <label key={c.id} className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-surface-2 cursor-pointer transition-colors">
...
export default function POS() {
  ...
  useEffect(() => { loadAll(); loadOrder(); }, [loadAll, loadOrder]);
  ...
  const addItem = async (item) => {
    ...
    try {
      ...
      await api.post(`/orders/${o.id}/items`, { menu_item_id: item.id, qty: newQty });
    ...
    <div className="flex h-full overflow-hidden">
      ...
      {showBill && order && (
        ...
        />
            ...
            <div className="w-36 flex-shrink-0 border-l border-surface-3 bg-surface-1 overflow-y-auto py-2 px-1.5 space-y-0.5">
        <button
          onClick={() => setSelCat(null)}
          className={`w-full px-2 py-2 rounded-lg text-xs font-medium text-left transition-all ${
        ...
        {categories.map(c => (
          <button
            ...
            className={`w-full px-2 py-2 rounded-lg text-xs font-medium text-left transition-all ${
              selCat === c.id ? 'bg-brand-500 text-white' : 'text-gray-400 hover:bg-surface-2 hover:text-gray-200'
            }`}
          >
            {c.name}
          </button>
...
<div className="w-64 xl:w-72 flex flex-col flex-shrink-0 border-r border-surface-3 overflow-hidden">
        <div className="h-11 flex items-center px-4 border-b border-surface-3 flex-shrink-0">
          <span className="font-semibold text-gray-100 text-sm flex-1">
            {order ? `Order #${order.bill_number}` : 'No active order'}
          </span>
        </div>
            ...
            (order.items || []).map(it => (
              <div key={it.id} className="flex items-center gap-2 px-3 py-2.5">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-500">₹{it.unit_price_snapshot.toFixed(2)}</p>
                </div>
                <div className="flex items-center gap-1 flex-shrink-0">
                  <button onClick={() => setQty(it.menu_item_id, it.qty - 1)}
                    className="w-6 h-6 rounded-md bg-surface-3 hover:bg-surface-4 text-gray-300 text-sm font-bold flex items-center justify-center">−</button>
        ...
        {/* Footer */}
        <div className="border-t border-surface-3 p-3 space-y-2 flex-shrink-0">
          ...
          <button
            onClick={clearOrder}
            disabled={!order}
            className="w-full text-xs text-gray-600 hover:text-red-400 transition-colors py-1 disabled:opacity-30"
          >
            Clear Order
          </button>
        </div>

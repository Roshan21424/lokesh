    ...
    <div className="flex flex-col h-screen overflow-hidden">
      ...
      <header className="h-12 flex-shrink-0 bg-surface-1 border-b border-surface-3 grid grid-cols-3 items-center px-4">
        ...
        <nav className="flex items-center justify-center gap-5">
          {NAV.map(({ to, label }) => (
            <NavLink
              ...
              to={to}
              className={({ isActive }) =>
                `px-2 py-1 rounded-sm text-xs font-semibold ${isActive ? "bg-brand-500 text-white" : "text-gray-400 hover:bg-surface-2 hover:text-white"}`
              }
            >
              {label}
            </NavLink>
        ...
        <div className="flex items-center justify-end gap-5">
          ...
          <button
            // Change the Exit button handler
            onClick={async () => {
              try {
                logout(); // add this — clears localStorage
